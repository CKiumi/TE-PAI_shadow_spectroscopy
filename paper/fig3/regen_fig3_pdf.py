"""Regenerate fig3-1 (seed80, M=1000) and fig3-2 (seed80, M=10000) as VECTOR PDFs from
the cached npz, replicating fig3_amplitude_damping.py's raw plot + settings box.

Run:  uv run python paper/fig3/regen_fig3_pdf.py
"""
import os
import numpy as np
...
# fig3 constants (must match fig3_amplitude_damping.py; seed shown as 80 for the seed-8 figs)
DELTA = np.pi / 2**6
...
def make(npz_name, cfg, out_stem, align_tepai_to_trotter=True, trotter_noisy_scale=1.0,
    ...
    if per_curve_norm:
        # normalise EVERY curve to its own peak -> all peaks = 1, so the noisy Trotter
        # appears at the same scale across panels (M=1000 vs M=10000). The collapse shows
        # via peak POSITION (Trotter noisy peaks at low freq, TE-PAI noisy at the gap).
        for lbl in STYLES:
            fr, sp = curves[lbl]
            curves[lbl] = (fr, sp / (np.abs(sp).max() or 1.0))
        print(f"  {out_stem}: per-curve normalisation (each curve -> own max)")
    elif align_tepai_to_trotter:
...
if __name__ == "__main__":
    ...
    make("fig3_seed80_M1000.npz", dict(base, M=1000,  trotter_shots=1000),  "fig3-1", per_curve_norm=True)
    make("fig3_seed80_M10000.npz", dict(base, M=10000, trotter_shots=10000), "fig3-2", per_curve_norm=True)
